    ...
    def robots_check(self):
        ...
        for ii in range(1,3):
            ...
            # First check for any special characters - protect them: '.', '?', '$', '*'
                # Aim to change targets, such as: /food/*/recipes into a 'regular expression'.
                ...
                # A '\' escapes the meaning from the RegEx - but we need to use raw strings as '\' could indicate other esc. sequences.
                ...
                # Important that we do this AFTER protections. Otherwise the '.' that we add gets protected and loses the RegEx meaning - which we want in this case.
                el = el.replace('*', '.*')
        ...
        # According to: http://www.robotstxt.org/robotstxt.html, 'everything not explicitly disallowed is considered fair game'
            ...
            # We could not strip trailing slashes. But that leads to inconsistencies in data storage and the problem in (%*).
